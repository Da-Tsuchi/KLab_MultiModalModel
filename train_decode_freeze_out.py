...
def train():
    ...
    # 2. モデルの語彙サイズを新しい語彙サイズに合わせて調整
    model.module.transformer.resize_token_embeddings(len(tgt_tokenizer))
    ...
    for epoch in range(args.start_epoch, args.num_epochs+1):
        ...
        for i, (src_images, tgt_images, src_texts, tgt_texts) in enumerate(train_loader):
            ...
            with autocast(dtype=torch.bfloat16):  # autocastコンテキストの開始
            ...
            scaled_loss = scaler.scale(loss) / args.accumulation_steps
            ...
            if (i + 1) % args.accumulation_steps == 0 or i + 1 == len(train_loader):
                # optimizer.step()
                # pbar.update(1)
                scaler.step(optimizer)  # スケーリングされた勾配でoptimizerを更新
                scaler.update()  # GradScalerの更新
                # optimizer.zero_grad()
